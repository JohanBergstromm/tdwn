...
function newsApi() {
    ...
    $('.selection-wrap').on('click', 'button#go', function(e) {
        ...
        $.get(
            'https://newsapi.org/v1/articles?source=' + selectedNewsOption + '&apiKey=0300c0ea3b734c0d8b418bf291c420a1'
        )
            .then(function(news) {
                for (var key in news) {
                }
                ...
                $('.content-wrap').fadeOut(function() {
                    ...
                    $('.result-wrap, .header .go-back').fadeIn();
                    $('.result-wrap + .go-back').addClass('active');
                });
            })
            .fail(function(err) {
                console.log(err);
...
function goBack() {
    $('.go-back').click(function() {
        ...
        $('.result-wrap + .go-back').removeClass('active');
        $('.result-wrap, .header .go-back').fadeOut(function() {
            $('.content-wrap').fadeIn();
        });
    });
}
